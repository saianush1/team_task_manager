...
// @desc    Register new user — first user is admin, all others are member
// @access  Public
router.post('/signup', [
  ...
  try {
    ...
    res.status(201).json({
      ...
      message: assignedRole === 'admin'
        ? 'Admin account created! You are the workspace admin.'
        : 'Account created! You can now request to join projects.',
      token,
      user
    });
  } catch (error) {
